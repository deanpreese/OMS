...
def save_models(generators, discriminator, gan, path="models"):
    ...
    for i, gen in enumerate(generators):
        gen.save(os.path.join(path, f'generator_{i}.h5'))
    
    discriminator.save(os.path.join(path, 'discriminator.h5'))
    gan.save(os.path.join(path, 'gan.h5'))
...
def load_models(path="models"):
    ...
    for i in range(3):  # Assuming 3 generators
        generators.append(load_model(os.path.join(path, f'generator_{i}.h5')))
    
    discriminator = load_model(os.path.join(path, 'discriminator.h5'))
    gan = load_model(os.path.join(path, 'gan.h5'))
