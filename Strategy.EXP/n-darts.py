...
def plot_results_m(actuals, predictions_h, predictions_b, last_x_rows=100):
...
# Plot results
def plot_results_s(actuals, predictions, last_x_rows=100):
...
def train_and_save_model(train_target, val_target, train_covariates, val_covariates, 
    ...
    #model_h.fit(series=train_target, val_series=val_target, 
    #          past_covariates=train_covariates, val_past_covariates=val_covariates)
    
    model_h.fit(series=train_target, val_series=val_target)
    
    
    model_h.save(model_save_path)

    model_b.fit(series=train_target, val_series=val_target)
...
def main():
    ...
    model_h, model_b = train_and_save_model(y_train, y_test, X_train, X_test,
        input_chunk_length, output_chunk_length, 
            n_epochs, num_stacks, num_blocks, num_layers, layer_widths, 
    ...
    predictions_h = model_h.predict(output_chunk_length -1)
    predictions_b = model_b.predict(output_chunk_length -1)
