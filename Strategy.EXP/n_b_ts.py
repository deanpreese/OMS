...
class NBeatsBlock(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        backcast = Lambda(lambda theta: theta[:, :self.backcast_length])(theta_b)
        forecast = Lambda(lambda theta: theta[:, self.backcast_length:])(theta_f)
...
def build_nbeats_model(input_shape, stack_types, nb_blocks_per_stack, forecast_length, backcast_length, thetas_dim):
    ...
    for stack_type in stack_types:
        for _ in range(nb_blocks_per_stack):
            ...
            forecast = Add()([forecast, block_forecast]) if forecast is not None else block_forecast
