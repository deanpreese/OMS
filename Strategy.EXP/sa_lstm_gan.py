...
from tensorflow.keras.layers import LSTM, Dense, LeakyReLU, BatchNormalization, Input, Dropout, TimeDistributed
...
def create_generator(input_shape, output_dim):
    model = Sequential()
    ...
    model.add(TimeDistributed(Dense(output_dim, activation='tanh')))
    return model
...
# Function to evaluate the GAN with DTW
def evaluate_gan(generator, X_test, y_test):
    predictions = generator.predict(X_test)
    print(f"Predictions shape: {predictions.shape}")
    print(f"y_test shape: {y_test.shape}")
    predictions = predictions[:, -1, :]  # Extract the last time step predictions
...
if __name__ == "__main__":
    ...
    generator = create_generator(input_shape=(seq_length, features.shape[1]), output_dim=y.shape[1])
    discriminator = create_discriminator(input_shape=(seq_length, features.shape[1]))
